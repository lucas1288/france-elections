  ...
  return (
    <>
      ...
      <button
        ...
        className="absolute inset-x-4 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-20 rounded-2xl bg-white/95 px-4 py-3 text-left shadow-lg ring-1 ring-black/5 backdrop-blur-sm"
      >
        {title && <p className="truncate text-sm font-bold text-gray-900">{title}</p>}

        <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-gray-400">
          Répartition des sièges
        </p>
        <p className="text-xs text-gray-500">{majorityLine}</p>
            ...
            return (
              <div key={r.party}>
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm text-gray-800">{r.label}</span>
                  <span className="shrink-0 text-xs text-gray-400">{r.party}</span>
        ...
        <div className="mt-2.5 flex items-center justify-end gap-1 text-xs font-medium text-blue-600">
          <span>Détails</span>
      ...
      <Drawer.Root open={open} onOpenChange={setOpen}>
        <Drawer.Portal>
          ...
          <Drawer.Content
            className="fixed inset-x-0 bottom-0 z-40 flex max-h-[82%] flex-col rounded-t-2xl bg-white shadow-[0_-4px_24px_rgba(0,0,0,0.16)] outline-none"
          ...
          >
            ...
            <div className="flex items-center px-4 pb-1 pt-1">
              <Drawer.Title className="text-base font-bold text-gray-900">Répartition des sièges</Drawer.Title>
              <button
                type="button"
                aria-label="Fermer"
            ...
            <p className="mt-1 border-t border-gray-100 px-4 pt-2.5 text-xs leading-relaxed text-gray-500">
              {majorityLine}
            </p>
            ...
            <div className="mt-2 min-h-0 flex-1 space-y-1 overflow-y-auto overscroll-contain px-2 pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
              ...
              {unattributed > 0 && (
                <div className="mt-1 flex items-center gap-2.5 rounded-lg border-t border-gray-100 px-2 py-2.5 pt-3">
                  <span className="h-3 w-3 shrink-0 rounded-full bg-slate-300" />
                  <span className="min-w-0 flex-1 truncate text-sm text-gray-500">Sièges non attribués</span>
                  <span className="w-20 shrink-0 text-right text-sm font-semibold text-gray-500">
