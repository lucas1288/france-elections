...
export function MobileDetailSheet({ electionData, communeData, circoData, palette }: Props) {
  ...
  return (
    <div
      className={`fixed inset-x-0 bottom-0 z-40 flex max-h-[72%] flex-col rounded-t-2xl bg-white shadow-[0_-4px_24px_rgba(0,0,0,0.16)] transition-transform duration-300 ${
    ...
    >
      {!commune ? (
        <div className="px-4 pb-8 pt-5">
          <p className="text-base font-bold text-gray-900">Chargement…</p>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-[calc(2.5rem+env(safe-area-inset-bottom))]">
          ...
          <div className="flex items-start gap-2 px-4 pt-4">
            <div className="min-w-0 flex-1">
              <h2 className="truncate text-lg font-bold text-gray-900">{commune.name}</h2>
