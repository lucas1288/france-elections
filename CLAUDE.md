...
- DesktopLayout top bar: GranularityToggle + `TerritorySearchBar` (geo-axis pill, opens `TerritoryNavigator`) + ThemeToggle — **view + geo controls only**. The old three-`<select>` `ElectionSelector` was removed (July 2026) in favour of the mobile-style picker, and the election chip + T1/T2 round toggle that replaced it were themselves REMOVED (July 20, 2026, lucas): the floating `TimelineStrip` (P4, bottom-centre of the map area) already carries the whole time axis — stops switch elections, pills switch rounds, and its list icon opens the shared `ElectionPicker` (centered modal on desktop). Mobile top bar is TWO rows: search pill above, `TimelineStrip` below (the strip likewise REPLACED the mobile election chip + T1/T2 row, July 2026)
