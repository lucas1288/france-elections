...
| top-right | zoom + theme util stack | **FranceMap** (needs `mapRef`) |
...
- Knock-ons: `METRO_FIT` top/bottom padding 40 → **80** (no header, so the fit must clear the floating chrome itself); the MapLibre `NavigationControl` is gone (replaced by our own stack, so zoom and theme read as one group); `OverseasInsets` moved to `top-16 bottom-28` + `max-h-full overflow-y-auto` on its inner column — bounded by the chrome that owns those corners, which also fixes a pre-existing defect where the 11-item column overflowed a short viewport and Polynésie/Nouvelle-Calédonie were unreachable; `AbroadMap` moved to `top-20` because the search pill is centred on the MAP AREA and reached into its column on narrower windows; `Hemicycle`'s own header got `md:pt-20` to start below the floating row (shifting it right doesn't work — the centred search occupies the middle of that row too).
- Still to come in R4: the overseas ORBIT replaces `OverseasInsets` + `AbroadMap` entirely, which also removes the west-coast overlap where métropole runs under the insets column at narrow widths.

### Mobile floating chrome (redesign R3, July 31 2026)
lucas moved the **timeline strip from the top to the bottom** — "consistent with the desktop one and easier to navigate on mobile devices", i.e. into the thumb zone. Everything else follows from that one move:

| Position | Control |
|---|---|
| top (header) | search pill — **ONE row** now (was search + strip) |
| top-right rail | `LayersMenu`, then FranceMap's zoom + theme stack below it |
| top-left | the round back-to-overview button (FranceMap) |
| bottom | `TimelineStrip`, pinned, floating-card like desktop |
...
- **`src/utils/mobileChrome.ts`** holds the band geometry (`UNDER_HEADER`, `UNDER_LAYERS`, `ABOVE_STRIP`, `HIDE_BELOW_STRIP`) as `calc()` strings applied via `style={{…}}`. They're constants, not Tailwind classes, because arbitrary-value classes can't be composed from a variable (Tailwind scans literal text). This exists because the old two-row header's `8.25rem` was hard-coded across four files and went stale the instant the header changed height — change a band here and every surface follows.
- Gotchas hit: (1) `translate-y-full` no longer hides a sheet anchored at a non-zero `bottom` — it only moves it down by its own height, leaving that offset's worth on screen; `HIDE_BELOW_STRIP` adds it back. (2) The mobile layers menu needs **z-50, above the detail sheet's z-40** — it opens downward into the sheet's band, so at or below the sheet's level it opens invisibly (the button lights up and nothing appears). (3) `HemicycleSheet`'s seats chip was bottom-left, i.e. under the strip; it moved into the right rail at `UNDER_LAYERS` (the zoom stack is hidden by the hemicycle cover, so that slot is free). (4) The `Hemicycle` title rendered UNDERNEATH the mobile header — invisible, and pre-existing; it now starts at `UNDER_HEADER` on both platforms with `pl-16` on mobile to clear the back button.
...
- DesktopLayout renders `FranceMap`, `AbroadMap` (top-right, `absolute top-20 right-16` since R3), `ResultsPanel`, plus the floating chrome. The old floating `Legend` ("En tête") was REMOVED (July 2026) — force selection lives in the sidebar's `NationalSummary` (mobile model)
...
- **`src/utils/panelTabs.ts`** — `usePanelTabs({ scope, code, isDeptSelection, nationalTerritories })` returns the tab LIST (order + labels), so both platforms iterate the same thing and only the bar's styling differs. **A tab is only offered when it has content**: Historique needs `useHasDeptHistory` (files loaded + ≥2 elections of the selected type — `src/utils/deptHistoryAvailable.ts`, kept out of `DeptHistory.tsx` so that file exports only a component and Fast Refresh keeps working); Territoires needs the 30-cities list (national), the dept insight sections, or a PLM whole-city's arrondissements. `nationalTerritories: false` on the mobile national sheet — there the cities live in the search sheet, and duplicating them would be a second copy to maintain.
...
- Knock-on cleanups: `PanelShell`'s standing "Résultats" `<h2>` became the panel's SUBJECT ("France entière" / the territory name) — a "Résultats" heading directly above a "Résultats" tab said the same word twice; and `NationalSummary` lost its collapsible "RÉSULTATS NATIONAUX" header for the same reason. `DeptInsight` no longer renders `<DeptHistory>` itself (callers place it in its own tab) and is now purely territorial. The desktop city list switches granularity to commune on click, so it works from the circo tab too.
